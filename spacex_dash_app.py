...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        df_each = spacex_df[spacex_df['Launch Site']==entered_site]
        success_rate=df_each['class'].mean()
        df_each_2=pd.DataFrame({'class':[0,1],'rate':[success_rate, 1.0-success_rate]})
        fig=px.pie(df_each_2, values='rate',
        names='class',
...
def get_scatter_plot(entered_site, entered_payload_range):
    min_payload, max_payload = entered_payload_range
    if entered_site=='ALL':
        ...
        df_each = spacex_df[spacex_df['Launch Site']==entered_site]
        mask=(df_each['Payload Mass (kg)']>=min_payload) & (df_each['Payload Mass (kg)']<=max_payload)
        fig=px.scatter(df_each[mask], x='Payload Mass (kg)', y='class',color='Booster Version Category',
